  ...
  if (type) {
    if (type === 'email') {
      ...
      verifyAndCallback(async (userinfo, accessToken) => {
        ...
        const userData = targetModel === 'teach'
          ? await userModel.findOneAndUpdate({ email: userinfo.email }, { $push: { likedCareerEdu: targetId } }, { new: true })
          : await userModel.findOneAndUpdate({ email: userinfo.email }, { $push: { likedMentor: targetId } }, { new: true });
