  ...
  try {
    ...
    if (req.cookies.authorization && req.cookies.authorization !== '') {
      ...
      if (!registered.sns || registered.sns.findIndex(ele => ele.snsType === type) === -1) {
        ...
        if (type === 'kakao') {
          const { data: tokenInfo } = await axios.get('https://kapi.kakao.com/v1/user/access_token_info', {
            headers: {
        ...
        userModel.findOneAndUpdate({ email: userData.email }, { $push: { sns: userData.sns[0] } })
          .then(() => {
