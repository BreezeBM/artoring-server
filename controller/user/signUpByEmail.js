...
module.exports = async (req, res) => {
  ...
  try {
    if (email) {
      const data = await userModel.findOne({ email }).select({ thumb: 1, nickName: 1, email: 1, isMentro: 1, likedCareerEdu: 1, likedMentor: 1, createdAt: 1 });
      ...
      if (data) {
        ...
        for (let i = 0; i < hashingTime; i++) {
          console.log(`${i}번째 해싱중... ${password}, ${typeof salt}`);
          password = await sha256Encrypt(999, password, salt);
          console.log('after: ', password, salt);
        }
